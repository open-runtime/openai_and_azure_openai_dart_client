...
abstract class UploadFileApi {
  ...
  ///
  /// [requestBody] - The definition of the file to create including its purpose and the location on the local machine.
  @POST('/uploads')
  Future<HttpResponse<UploadResource>> uploadFileStart({
    @Body() required UploadFileStartBody requestBody,
  ...
  Future<HttpResponse<UploadResource>> uploadFileComplete({
    @Path('upload-id') required String uploadId,
    @Body() required UploadFileCompleteBody requestBody,
    @Query('api-version') String? apiVersion,
    @Extras() Map<String, dynamic>? extras,
    @DioOptions() RequestOptions? options,
  });
