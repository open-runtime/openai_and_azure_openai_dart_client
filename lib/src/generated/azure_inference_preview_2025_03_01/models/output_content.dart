...
@MappableClass(
  ...
  includeSubClasses: [OutputContentOutputText, OutputContentRefusal],
)
...
extension OutputContentUnionDeserializer on OutputContent {
  static OutputContent tryDeserialize(Map<String, dynamic> json, {String key = 'type', Map<Type, Object?>? mapping}) {
    final mappingFallback = const <Type, Object?>{OutputText: 'output_text', Refusal: 'refusal'};
    final value = json[key];
    final effective = mapping ?? mappingFallback;
    return switch (value) {
      _ when value == effective[OutputText] => OutputTextMapper.fromJson(json),
      _ when value == effective[Refusal] => RefusalMapper.fromJson(json),
...
class OutputContentOutputText extends OutputContent with OutputContentOutputTextMappable {
  ...
  const OutputContentOutputText({required this.type, required this.text, required this.annotations});
}
