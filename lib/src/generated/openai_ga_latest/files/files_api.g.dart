...
class _FilesApi implements FilesApi {
  ...
  Future<HttpResponse<ListFilesResponse>> listFiles({
    ...
    late ListFilesResponse _value;
    try {
      _value = ListFilesResponse.fromJson(_result.data!);
    } on Object catch (e, s) {
  ...
  Future<HttpResponse<OpenAiFile>> createFile({
    ...
    _data.fields.add(
      MapEntry(
        'expires_after',
        jsonEncode(expiresAfter ?? <String, dynamic>{}),
      ),
    );
    final newOptions = newRequestOptions(options);
    newOptions.extra.addAll(_extra);
    ...
    final _result = await _dio.fetch<Map<String, dynamic>>(_options);
    late OpenAiFile _value;
    try {
      _value = OpenAiFile.fromJson(_result.data!);
    } on Object catch (e, s) {
      errorLogger?.logError(e, s, _options, _result);
      rethrow;
    }
