...
class ProjectCreateRequestMapper extends ClassMapperBase<ProjectCreateRequest> {
  ...
  @override
  final MappableFields<ProjectCreateRequest> fields = const {
    #name: _f$name,
  ...
  static ProjectCreateRequest _instantiate(DecodingData data) {
    return ProjectCreateRequest(
      ...
      geography: data.dec(_f$geography),
    );
  }
...
class _ProjectCreateRequestCopyWithImpl<$R, $Out>
  ...
  @override
  $R call({String? name, Object? geography = $none}) => $apply(
    FieldCopyWithData({
      if (name != null) #name: name,
      if (geography != $none) #geography: geography,
    }),
  );
  @override
  ProjectCreateRequest $make(CopyWithData data) => ProjectCreateRequest(
    name: data.get(#name, or: $value.name),
    geography: data.get(#geography, or: $value.geography),
  );
