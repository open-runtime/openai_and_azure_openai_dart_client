...
class _AudioApi implements AudioApi {
  ...
  Future<HttpResponse<CreateTranscriptionResponseUnion>> createTranscription({
    ...
    TranscriptionChunkingStrategy? chunkingStrategy,
    ...
    _data.fields.add(
      MapEntry(
        'chunking_strategy',
        jsonEncode(chunkingStrategy ?? <String, dynamic>{}),
      ),
    );
    knownSpeakerNames?.forEach((i) {
      _data.fields.add(MapEntry('known_speaker_names', i));
    });
    knownSpeakerReferences?.forEach((i) {
      _data.fields.add(MapEntry('known_speaker_references', i));
    });
    final newOptions = newRequestOptions(options);
    newOptions.extra.addAll(_extra);
    newOptions.headers.addAll(_dio.options.headers);
